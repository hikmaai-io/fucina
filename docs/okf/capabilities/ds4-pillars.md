...
| **D — session persistence** | Implemented; live HTTP GPU gate pending | Versioned checked snapshot format; Gemma REPL/HTTP; Qwen paged REPL plus continuous-batching HTTP restore/export with GDN state | Run and retain the Qwen HTTP restart hardware gate |
...
The original “80% there” architecture plan is now largely realized for one-node Qwen execution. The remaining distance is not the core model forward: it is proving an advantageous precision policy, adding structured output across mandatory batching, recording the Qwen HTTP persistence hardware gate, and building the real engine boundary for distributed execution.
