...
17. **Added live Qwen MoE prefill phase telemetry.** `--timings` enables diagnostic CUDA events around expert dequant, router/route/gather, grouped experts, shared expert, and the rest of the layer pipeline. A warm 2013-token MoE-35B prefill measured 1376.9 ms: 368.3 / 51.5 / 372.0 / 27.1 / 558.0 ms respectively. The old 71.6%-expert/11.6%-router prior no longer describes the grouped-GEMM path.
18. **Made admission page-cache-aware and context-proportional.** Qwen uses Linux `MemAvailable` for the physical growth gate and exact model+runtime accounting for the util gate. Slot planning reserves 8192 tokens by default (`FUCINA_QWEN35_SLOT_CTX` restores/configures the policy), while geometric FULL KV may grow transactionally to maxctx and fails cleanly before mutation if headroom vanishes.
19. **Removed the stale 16-slot CLI cap.** Qwen can now expose the engine's full 32-row capacity. Live at maxctx 25,280: slots=32/32; four concurrent 8.5K-token requests crossed the typical reservation successfully, followed by a diverse C32 burst with 32/32 HTTP 200 and no admission failures.
20. **Closed the scheduler-admission investigation.** Main's existing lockstep idle-burst coalescing already admits short bursts to capacity; busy one-shot admissions intentionally remain one/pass to protect decode, while long requests use chunked/fused row budgeting. A host token counter around sequential `AddSeq` would add stalls without creating a GPU-batched prefill, so no redundant policy was added.
...
**Acceptance:** configured capacity reserves the documented typical context per slot. Growth beyond that reservation is transactional and checks current physical/policy headroom before pointer publication; allocation pressure produces a graceful sequence stop, never OOM, corruption, or partial state.

#### 1.3 Account for all lazy memory — implemented for Qwen model/state/weight-cache
