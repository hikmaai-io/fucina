...
| # | Item | Outcome |
|---|---|---|
| 1 | **json_schema / response_format** | ✅ **SHIPPED**, including mandatory batching (2026-07-26). The original grammar/schema subset remains; the scheduler now carries an independent FSM per slot, uses exact host-input decode/full-logit masking, and cleanly disables spec on constrained shared steps. Qwen3.6-35B-A3B-FP8 TC-64/65/66/67/69-style requests passed 5/5 schema validation. See `docs/batched-structured-output.md`. |
