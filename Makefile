...
# + grouped K-norm + neox RoPE) vs a host double reference reading the same bf16 tensors. SKIPs if absent.
...
# P3 non-causal GQA attention parity (the DFlash query forward's new attention pattern): device fp32
# vs host double softmax reference on the real draft geometry (NQ=32/NKV=8/HD=128). Self-contained.
qwen35-dflash-attn-parity-test:
	$(NVCC) -O3 -arch=$(CUDA_ARCH) -std=c++17 -Icuda cuda/test_qwen35_dflash_attn_parity.cu -o /tmp/dflash_attn \
		-lcudart -lcuda
...
# DFlash full query forward (the true drafting forward): precompute context K/V then (1+K) query
# rows attend context ++ own block non-causally through all layers, vs host double. SKIPs if absent.
qwen35-dflash-query-test:
	$(NVCC) -O3 -arch=$(CUDA_ARCH) -std=c++17 -Icuda cuda/test_qwen35_dflash_query.cu -o /tmp/dflash_query \
		-lcudart -lcuda
...
# DFlash aux-hidden combine (fc): the target->draft input interface, fc(concat of F target-layer
# hidden states) -> draft input hidden, over residency vs host double on the real weights. SKIPs if absent.
qwen35-dflash-combine-test:
	$(NVCC) -O3 -arch=$(CUDA_ARCH) -std=c++17 -Icuda cuda/test_qwen35_dflash_combine.cu -o /tmp/dflash_combine \
		-lcudart -lcuda
	flock -w 600 /tmp/fucina_gpu.lock -c "/tmp/dflash_combine"

# DFlash single drafting entry point (fc -> precompute -> query forward -> greedy sample): produces
# K in-vocab draft tokens and is run-to-run byte-identical, composed over real weights. SKIPs if absent.
qwen35-dflash-draft-test:
	$(NVCC) -O3 -arch=$(CUDA_ARCH) -std=c++17 -Icuda cuda/test_qwen35_dflash_draft.cu -o /tmp/dflash_draft \
		-lcudart -lcuda
...
qwen35-dflash-aux-gather-test:
	$(NVCC) -O3 -arch=$(CUDA_ARCH) -std=c++17 -Icuda cuda/test_qwen35_dflash_aux_gather.cu -o /tmp/dflash_auxg \
		-lcudart -lcuda
...
qwen35-dflash-e2e-test: lib libdg
	$(NVCC) -O3 -arch=$(CUDA_ARCH) -std=c++17 -Icuda cuda/test_qwen35_dflash_e2e.cu \
		cuda/libfucina.a cuda/libdg.a -o /tmp/fucina_dflash_e2e \
		-lcudart -lcublas -lcublasLt -lcuda -lpthread -lstdc++ -lm
