...
// concrete engine implements it, the scheduler drafts tokens per slot and verifies
// them in ONE batched forward via StepBatchSpec — committing the accepted run per
// slot (one weight pass, multiple tokens) instead of one token per slot. out[i] is
// the committed run for reqs[i].Slot, in request order; a run beginning with the -1
// sentinel signals that slot must stop (KV could not grow), exactly like StepBatch.
type SpecBatchEngine interface {
	BatchEngine
	StepBatchSpec(reqs []SpecReq) (out [][]int32, err error)
}

// MaxVerifyRows is the engine's per-step verify-row budget (GEMMA4_MAX_SEQS): the
// speculative step flattens to Σ(1+len(drafts)) forward rows, which must not exceed
// it. With R active slots, R rows are anchors and the rest is the draft budget — so
// speculation naturally tapers as concurrency rises (anchors crowd out drafts),
// which matches where spec pays off (low concurrency / single-stream latency).
...
func New(engine BatchEngine, queueDepth int) *Scheduler {
	...
	// max per-slot draft length, kept under GEMMA4_SPEC_MAX-1 so a full draft plus
	// its anchor fits the verify row budget.
...
func (s *Scheduler) step(active map[int]*seq) bool {
	...
	// Speculative fast path when the engine supports it AND there is draft budget.
	if s.spec != nil {
...
// stepSpec is the speculative analogue of step: per active slot it drafts a few
// tokens (prompt-lookup over that sequence's history) and verifies them all in ONE
// batched forward via the engine's StepBatchSpec, then scatters the committed RUN
// (accepted drafts + bonus token) back to each sequence. The accepted run is what
// makes spec pay off — one weight pass commits multiple tokens. The verify flattens
// to Σ(1+drafts) rows ≤ MaxVerifyRows: R anchors are mandatory, the rest is the
// draft budget, distributed greedily, so speculation tapers as concurrency rises.
func (s *Scheduler) stepSpec(active map[int]*seq) bool {
	...
	// No slot produced a draft (e.g. all novel prose): the speculative verify would
	// be pure overhead vs a plain one-token step. Fall back so we never pay to "spec"
	// zero drafts. (s.spec != nil still, so we call the engine's plain StepBatch.)
	if !anyDraft {
		return s.stepPlain(active, slots)
	}
	...
	// Scatter each committed run to its slot; deliver walks the run token-by-token
	// and stops emitting the moment a token evicts the sequence (stop/budget/cancel
	// or the -1 KV-exhausted sentinel) — the rest of that run is then dropped.
	for i, slot := range slots {
		sq := active[slot]
		if sq == nil {
...
// stepPlain runs one NON-speculative batched decode over the given slots (the body
// of step, factored out so stepSpec can fall back to it when no slot has a draft).
func (s *Scheduler) stepPlain(active map[int]*seq, slots []int) bool {
	si := make([]int32, len(slots))
	inputs := make([]int32, len(slots))
	for i, slot := range slots {
		si[i] = int32(slot)
		inputs[i] = active[slot].next
	}
	out, err := s.engine.StepBatch(si, inputs)
	if err != nil {
		log.Printf("batch: StepBatch failed (%d active): %v", len(slots), err)
		for _, slot := range slots {
			if sq := active[slot]; sq != nil {
				s.evict(active, sq, Result{Reason: FinishError, Generated: sq.generated, Err: err})
			}
